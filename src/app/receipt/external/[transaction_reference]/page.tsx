...
import PrintButtonClient from '../../print-button-client'; // Adjusted path

// Define types for fetched data (specific to external sales)
...
type ExternalSaleItemForReceipt = {
  id: string; external_sale_id: string; product_name: string; product_category_name: string | null;
  product_unit_name: string; quantity: number; unit_sale_price: number;
  unit_purchase_price_negotiated: number; total_cost: number; total_price: number; note: string | null;
};

type ExternalSaleRecordForReceipt = {
  ...
  payment_method: string; status: 'completed' | 'pending_approval' | 'cancelled';
  transaction_reference: string; created_at: string; updated_at: string;
  authorized_by_user_id: string | null; authorization_code_hashed: string | null;
...
export default async function ExternalReceiptPage({ params }: ExternalReceiptPageProps) {
  ...
  // Fetch external sale details
  const { data: saleResult, error: saleError } = await supabase
    .from('external_sales')
    .select(`
      *,
      users!external_sales_cashier_id_fkey(id, email), -- Alias for cashier
      branches(id, name, location),
      authorized_by_user:users!external_sales_authorized_by_user_id_fkey(id, email), -- Alias for manager
      external_sale_items(*)
    `)
  ...
  const sale: ExternalSaleRecordForReceipt = saleResult as unknown as ExternalSaleRecordForReceipt;
  ...
  const receiptPhrases: ReceiptPhrase[] = (receiptPhrasesData as unknown as ReceiptPhrase[]) || [];
  ...
  // Calculate Total Units Sold (for external sale)
  const totalUnitsSold = sale.external_sale_items.reduce((sum, item) => sum + item.quantity, 0);
  const totalExternalCost = sale.external_sale_items.reduce((sum, item) => sum + item.total_cost, 0);
        ...
        <div className="p-6 md:p-8">
          <div className="flex justify-between items-start mb-6">
            ...
            <div className="text-right">
              <h2 className="text-xl font-semibold text-gray-800">EXTERNAL RECEIPT</h2>
              ...
              {sale.authorized_by_user && <p className="text-sm text-gray-600">Authorized By: {sale.authorized_by_user.email}</p>}
            </div>
          </div>
            ...
            <Table className="w-full text-left table-fixed">
              <TableHeader>
                <TableRow className="bg-gray-100">
                  ...
                  <TableHead className="w-[30%]">Product</TableHead> {/* Adjusted width */}
                  <TableHead className="w-[10%] text-center">Qty</TableHead>
                  <TableHead className="w-[10%] text-center">Unit</TableHead>
                  <TableHead className="w-[15%] text-right">Sale Price</TableHead> {/* Renamed */}
                  <TableHead className="w-[15%] text-right">Partner Cost</TableHead> {/* NEW: Partner Cost */}
                  <TableHead className="w-[15%] text-right">Line Total</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {sale.external_sale_items.length > 0 ? (
                  sale.external_sale_items.map((item: ExternalSaleItemForReceipt, idx: number) => (
                    <React.Fragment key={item.id}>
                      <TableRow className="border-b border-gray-200">
                        <TableCell className="text-sm">{idx + 1}</TableCell>
                        <TableCell className="font-medium text-gray-900">
                          {item.product_name} {item.product_category_name && `(${item.product_category_name})`}
                          {item.note && <span className="block text-xs italic text-gray-500">Note: {item.note}</span>}
                        </TableCell>
                        <TableCell className="text-center">{item.quantity}</TableCell>
                        <TableCell className="text-center">{item.product_unit_name}</TableCell>
                        <TableCell className="text-right">{formatCurrency(item.unit_sale_price)}</TableCell>
                        <TableCell className="text-right">{formatCurrency(item.unit_purchase_price_negotiated)}</TableCell> {/* Display Partner Cost */}
                  ...
                  <TableRow>
                    <TableCell colSpan={7} className="h-16 text-center text-gray-500">No items found for this external sale.</TableCell>
                  </TableRow>
                )}
                {/* Total Units and Costs for External Sale */}
                <TableRow className="font-bold border-t border-gray-400">
                    ...
                    <TableCell colSpan={2} className="text-right py-2">Total Partner Cost:</TableCell> {/* NEW: Total Partner Cost */}
                    <TableCell className="text-right py-2">{formatCurrency(totalExternalCost)}</TableCell>
                </TableRow>
              </TableBody>
            </Table>
          </div>
          ...
          <div className="flex justify-end mb-6 text-lg font-bold">
            <div className="w-full md:w-1/2">
              <div className="flex justify-between border-b pb-2 mb-2">
                <span>Total Sale Amount:</span> {/* Renamed from Subtotal */}
                <span>{formatCurrency(sale.total_amount)}</span>
              </div>
              <div className="flex justify-between text-2xl">
                <span>Net Profit (External):</span> {/* NEW: Net Profit for External Sale */}
                <span>{formatCurrency(sale.total_amount - totalExternalCost)}</span>
              </div>
            </div>
          </div>
